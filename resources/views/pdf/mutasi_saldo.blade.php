...
<body>
    <div class="container">
            ...
            <div class="company-info" style="margin-right: 17pc;">
                ...
                <div class="company-info">
                    <table>
                        ...
                        <br>
                        <p><b>
                                {{ $profile->getKabupaten->name }}, {{ date('d-m-Y', strtotime($mutasi->tgl_input)) }}
                            </b></p>
                        <tr>
                            <td>No. Reff </td>
                            <td>: {{ $mutasi->no_reff }}</td>
                        </tr>
                    </table>
                </div>
            </div>
        ...
        <div class="title">
            <h2>BUKTI MUTASI SALDO</h2>
        </div>
        <table class="table">
            <thead>
                <tr>
                    <th>Saldo Berkurang</th>
                    <th>Nominal</th>
                    <th>Saldo Bertambah</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>{{ $mutasi->akunPengirim->nama_akun }}</td>
                    <td>Rp. {{ number_format($mutasi->jumlah_saldo, 0, ',', '.') }}</td>
                    <td>{{ $mutasi->akunPenerima->nama_akun }}</td>
                </tr>
            </tbody>
        </table>
        <div class="invoice-hal">
            <p>
                <b>Keterangan : {{ $mutasi->keterangan }}</b>
            </p>
        </div>
