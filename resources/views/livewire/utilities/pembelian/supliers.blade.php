<div>
    <div class="grid grid-cols-12 gap-2 mt-8">
        ...
        <div class="col-span-6 md:col-span-3">
            ...
            <div class="flex items-center mb-2">
                <select data-tw-merge id="sp" name="sp" wire:model.live="spId"
            ...
            {{-- <div class="col-span-6 md:col-span-3"style="position: absolute; top: 340px; right:50px;">
                <div class="flex gap-2">
                    <button class="btn btn-success">
                        <span wire:ignore><i data-feather="file-text" class="w-5 h-5 mr-1"></i>Excel</span>
                    </button>
                    <a href="{{ route('cetak_pdf_suplier_beli', [
                        'search' => $search,
            ...
            <table class="table mt-5">
                ...
                <thead>
                    <tr class="header-gray">
                        ...
                        <th rowspan="2" class="border border-slate-600">Tanggal Faktur</th>
                        ...
                        <th rowspan="2" class="border border-slate-600">Hutang</th>
                ...
                <tbody>
                    @forelse ($pembelians as $pembelian)
                        <tr class="text-center intro-x">
                            ...
                            <td class="border border-slate-600">{{ date('d-m-Y', strtotime($pembelian->tgl_input)) }}
                            </td>
                            <td class="border border-slate-600">{{ date('d-m-Y', strtotime($pembelian->tgl_faktur)) }}
                            </td>
                            <td class="border border-slate-600">{{ $pembelian->no_faktur }}</td>
                            <td class="border border-slate-600">{{ number_format($pembelian->subtotal, 0, ',', '.') }}
                            </td>
                            <td class="border border-slate-600">{{ $pembelian->diskon }}</td>
                            <td class="border border-slate-600">
                                {{ number_format($pembelian->hasil_diskon, 0, ',', '.') }}</td>
                            ...
                            <td class="border border-slate-600">{{ $pembelian->biaya1 }}</td>
                            <td class="border border-slate-600">{{ $pembelian->biaya2 }}</td>
                            <td class="border border-slate-600">
                                {{ number_format($pembelian->total_tagihan, 0, ',', '.') }}</td>
                            <td class="border border-slate-600">
                                {{ number_format($pembelian->jumlah_bayar, 0, ',', '.') }}</td>
                            <td class="border border-slate-600">
                                {{ number_format($pembelian->total_tagihan - str_replace('.', '', $pembelian->jumlah_bayar), 0, ',', '.') ?? '-' }}
                            </td>
                            <td class="border border-slate-600">
                                {{ $pembelian->kredit == 0 ? 'Cash' : 'Kredit' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
