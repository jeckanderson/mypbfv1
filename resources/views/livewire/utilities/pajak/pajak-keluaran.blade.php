<div class="grid grid-cols-12 gap-2 mt-8">
    ...
    <div class="col-span-12 overflow-auto intro-y lg:overflow-visible">
        <div class="gap-2 sm:flex">
            <div data-tw-merge class="flex items-center block gap-1">
            ...
            <div class="flex gap-2 mb-2 overflow-auto sm:mb-0">
                ...
                <input data-tw-merge id="horizontal-form-1" name="start" type="date" placeholder="Stok dari master"
                ...
                <input data-tw-merge id="horizontal-form-1" name="end" type="date" placeholder="Stok dari master"
            ...
            <button class="flex items-center text-white btn btn-md btn-success" style="position: fixed; right: 55px;">
                <a
                    href="{{ route('csv-pajak-keluaran', ['start' => $startDate, 'end' => $endDate, 'pelanggan' => $pelanggan]) }}">
                    <div class="flex gap-1 text-center">
                        <span wire:ignore> <i data-feather="file-text" class="w-4 h-4 mr-1"></i></span>
                        <p>Ekspor CSV</p>
                    </div>
                </a>
            </button>
                ...
                <tbody>
                    @forelse ($penjualans as $penjualan)
                        <tr class="text-center intro-x">
                            ...
                            <td class="border border-slate-600 ">{{ $penjualan->tgl_faktur->format('d-m-Y') }}</td>
                            <td class="border border-slate-600 ">{{ $penjualan->no_faktur }}</td>
                            <td class="border border-slate-600 ">{{ $penjualan->getPelanggan->nama }}</td>
                            <td class="border border-slate-600 ">{{ $penjualan->no_seri_pajak }}</td>
                            <td class="border border-slate-600 ">{{ $penjualan->tgl_input->format('d-m-Y') }}</td>
                            <td class="border border-slate-600 ">{{ number_format($penjualan->dpp, 0, ',', '.') }}</td>
                            <td class="border border-slate-600 ">{{ number_format($penjualan->ppn, 0, ',', '.') }}</td>
                            <td class="border border-slate-600 ">
                                {{ number_format($penjualan->dpp + $penjualan->ppn, 0, ',', '.') }}
                            </td>
                        </tr>
                    @empty
                    @endforelse
                    <tr>
                        ...
                        <td class="font-bold text-left border border-slate-600">
                            {{ number_format($penjualans->sum('dpp'), 0, ',', '.') }}</td>
                        <td class="font-bold text-left border border-slate-600">
                            {{ number_format($penjualans->sum('ppn'), 0, ',', '.') }}</td>
                        <td class="font-bold text-left border border-slate-600">
                            {{ number_format($penjualans->sum('ppn') + $penjualans->sum('dpp'), 0, ',', '.') }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
