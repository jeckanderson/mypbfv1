            ...
            @foreach ($item->piutang as $hutang)
                @if (!is_null($hutang->sourceable))
                    ...
                    <tr style="text-align: center;">
                        ...
                        <td>{{ $first->detailable_type == \App\Models\ReturPenjualan::class ? $first->detailable->total : 0 }}
                        </td>
                        <td>{{ $first->detailable_type != \App\Models\ReturPenjualan::class ? str_replace('.', '', $first->nominal_bayar) : 0 }}
                        </td>
                        <td>{{ str_replace('.', '', $first->sisa_hutang) }}</td>
                        <td>{{ $first->detailable_type == \App\Models\ReturPenjualan::class ? $first->detailable->uang_retur : 0 }}
                        </td>
                        <td>{{ str_replace('.', '', $first->sisa_hutang) }}</td>
                    ...
                    @foreach ($hutang->sourceable->piutang_pengguna as $item)
                        ...
                        <tr style="text-align: center;">
                            ...
                            <td>{{ $item->detailable_type == \App\Models\ReturPenjualan::class ? $item->detailable->total : 0 }}
                            </td>
                            <td>{{ str_replace('.', '', $item->nominal_bayar) }}</td>
                            <td>{{ str_replace('.', '', $item->sisa_hutang) }}</td>
                            <td>{{ $item->detailable_type == \App\Models\ReturPenjualan::class ? $item->detailable->uang_retur : 0 }}
                            </td>
                            <td>{{ str_replace('.', '', $item->sisa_hutang) }}</td>
