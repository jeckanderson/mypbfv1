...
<body>
    @if ($detail !== null && count($detail) > 0)
        <h1 style="text-align: center;">LAP. KARTU PIUTANG</h1>
            ...
            <tbody>
                @foreach ($detail as $piutangs)
                    <tr class="text-center intro-x">
                        ...
                        <td class="border border-slate-600">{{ $piutangs->tgl_input }}</td>
                        <td class="border border-slate-600">{{ $piutangs->no_faktur }}</td>

                        <td class="border border-slate-600">
                            {{ $piutangs->pembelian->tempo_kredit }}</td>
                        <td class="border border-slate-600">{{ $piutangs->total_piutang }}</td>
